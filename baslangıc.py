...
for e in epoch_iterator:
    ...
    for i, (u, yd) in enumerate(zip(x_egitim, yd_egitim)):
        ...
        weights_x = weights_x + learning_rate * \
            np.dot(((np.dot(weights_y.T, e))*derivative_v_to_x(V_k)), x_k.T)
        weights_u = weights_u + learning_rate * \
            np.dot(((np.dot(weights_y.T, e))*derivative_v_to_x(V_k)), u)
...
plt.plot(range(len(test_karesel_hata)), np.reshape(
    test_karesel_hata, (len(test_karesel_hata))))
...
ort_test_hatası = np.sum(hatalar) / len(hatalar)
plt.title('Eğitim Kümesi Ortalama Karesel Hata = ' + str(ort_test_hatası))
